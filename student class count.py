...
def print_selection():
    ''' Print out value of ListBox selection '''
...
def class_count():
    ...
    for i in student_list:
        if class_combo.value in i.get_classes():
            total+=1
        heading.value="Total Students: "+str(total)
...
def search():
    ...
    for student in student_list:
        if search_textbox.value in student.get_name():
...
class_combo = Combo(app, options=["GRA", "ENG", "PHY", "MAT", "ART"], command=class_count)
